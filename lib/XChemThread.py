    ...
    def run(self):
        ...
        for directory in sorted(glob.glob(os.path.join(self.initial_model_directory, '*'))):
            ...
            #                    os.symlink('dimple.pdb', 'refine.pdb')
                ...
                os.system(
                    '/bin/rm dimple.pdb 2> /dev/null')  # this makes sure that any broken link which could rerail PANDDA gets removed
            if os.path.isfile('dimple.mtz'):
                ...
                os.system(
                    '/bin/rm dimple.mtz 2> /dev/null')  # this makes sure that any broken link which could rerail PANDDA gets removed
            ...
            pandda_models = self.db.execute_statement(
                "select CrystalName,PANDDA_site_index,PANDDA_site_spider_plot,PANDDA_site_event_map from panddaTable where CrystalName='{0!s}'".format(
                    xtal))
            if not pandda_models == []:
                for entry in pandda_models:
                    ...
                    if entry[3] is not None:
                        event_map = os.path.join(self.initial_model_directory, xtal,
                                                 entry[3].split('/')[len(entry[3].split('/')) - 1])
                        if os.path.isfile(event_map):
                            db_pandda_dict['PANDDA_site_event_map'] = event_map
                    if entry[2] is not None:
                        spider_plot = os.path.join(self.initial_model_directory, xtal,
                                                   entry[2].split('/')[len(entry[2].split('/')) - 3],
                                                   entry[2].split('/')[len(entry[2].split('/')) - 2],
                                                   entry[2].split('/')[len(entry[2].split('/')) - 1])
...
class synchronise_db_and_filesystem(QtCore.QThread):
    ...
    def run(self):
        ...
        if self.mode != 'project_directory':
            ...
            if len(glob.glob(os.path.join(self.initial_model_directory, '*'))) != 0:
                progress_step = 100 / float(len(glob.glob(os.path.join(self.initial_model_directory, '*'))))
            self.Logfile.insert('found ' + str(
                len(glob.glob(os.path.join(self.initial_model_directory, '*')))) + ' samples in project directory')
            for directory in sorted(glob.glob(os.path.join(self.initial_model_directory, '*'))):
                try:
                    os.chdir(directory)
                except OSError:
        ...
        if os.path.isfile('refine.mtz'):
            if os.path.isfile(xtal + '.free.mtz'):
                ...
                if os.path.isfile(xtal + '.mtz'):
                    ...
                    self.Logfile.insert('%s: calculating CC between %s.free.mtz (%s refl) and %s.mtz (%s refl): %s' % (
                        ...
                        if float(CC) < 0.999:
                            ...
                            if os.path.isfile('dimple/dimple_rerun_on_selected_file/dimple/dimple.log'):
                                ...
                                if os.path.isfile(mtzin):
                                    self.Logfile.insert(
                                        '%s: mtzfile used for refinement is not the same as the one chosen from autoprocessing' % xtal)
                                    self.Logfile.insert('%s: current mtzfile after autoprocessing: %s' % (
    ...
    def sync_dimple_results(self, xtal, db_dict):
        ...
        if os.path.isfile(
                os.path.join(dimple_path, 'dimple', 'dimple_rerun_on_selected_file', 'dimple_run_in_progress')):
                ...
                if readindApoStructures:
                    if 'Pickling Object: processed_datasets' in line:
                        if line.split() >= 2:
                            ...
                            if os.path.isfile(
                                    os.path.join(panddaPATH, 'processed_datasets', xtal, xtal + '-pandda-input.pdb')):
    ...
    def sync_pandda_table_NEW(self):
        ...
        if pandda_models != []:
            for entry in pandda_models:
                ...
                panddaPATH = entry[6]
                apoStructures = entry[7]
                self.emit(QtCore.SIGNAL('update_status_bar(QString)'),
                          'checking {0!s} -> site {1!s} -> event {2!s} '.format(xtal, site_index, event_index))
                self.emit(QtCore.SIGNAL('update_progress_bar'), progress)
                progress += progress_step
                ...
                if str(apoStructures) == 'None' or apoStructures == '':
                    if panddaPATH != 'None' or panddaPATH != '':
                        self.Logfile.insert(
                            'trying to find which apo structures were used to calculate the event maps in ' + panddaPATH)
                        db_pandda_dict['ApoStructures'] = self.find_apo_structures_for_PanDDA(panddaPATH)
                    else:
                        self.Logfile.insert('pandda path for ' + xtal + ' is empty in database')
                ...
                if os.path.isfile(os.path.join(self.initial_model_directory, xtal, 'refine.pdb')):
                    ligands_in_file = pdbtools(
                        os.path.join(self.initial_model_directory, xtal, 'refine.pdb')).find_xce_ligand_details()
                    if ligands_in_file == []:
                        self.Logfile.warning('{0!s}: could not find any ligands in refine.pdb'.format(xtal))
                        continue
                    else:
                        self.Logfile.insert('{0!s}: found the following ligands in refine.pdb: {1!s}'.format(xtal, str(
                            ligands_in_file)))
                    ...
                    for ligand in ligands_in_file:
                        ...
                        residue_number = ligand[2]
                        residue_altLoc = ligand[3]
                        residue_xyz = pdbtools(os.path.join(self.initial_model_directory, xtal,
                                                            'refine.pdb')).get_center_of_gravity_of_residue_ish(
                            residue_chain, residue_number)
                        distance = misc().calculate_distance_between_coordinates(residue_xyz[0], residue_xyz[1],
                                                                                 residue_xyz[2], event_x, event_y,
                                                                                 event_z)
                        ...
                        self.Logfile.insert(
                            '{0!s}: calculating distance between event and ligand ({1!s} {2!s} {3!s}): {4!s}'.format(
                                xtal, residue_name, residue_chain, residue_number, str(distance)))
                    ...
                    self.Logfile.insert(
                        '{0!s}: ligand with the shorted distance ({1!s}A) to the current event (id: {2!s}): {3!s} {4!s} {5!s} {6!s}'.format(
                            xtal, str(distance), event_index, residue_name, residue_chain, residue_number,
                            residue_altLoc))
                    db_pandda_dict['PANDDA_site_ligand_resname'] = residue_name
                    db_pandda_dict['PANDDA_site_ligand_chain'] = residue_chain
                    db_pandda_dict['PANDDA_site_ligand_sequence_number'] = residue_number
                    ...
                    if xtal + '/Refine_' in os.path.realpath(
                        ...
                        tmp = os.path.realpath(os.path.join(self.initial_model_directory, xtal, 'refine.pdb'))
                        spider_plot = os.path.join(tmp[:tmp.rfind('/')], 'residue_plots',
                                                   residue_chain + '-' + residue_number + '.png').replace(' ', '')
                        if os.path.isfile(spider_plot):
                            db_pandda_dict['PANDDA_site_spider_plot'] = os.path.realpath(spider_plot)
                        if os.path.isfile(os.path.join(tmp[:tmp.rfind('/')], 'residue_scores.csv')):
                            with open(os.path.join(tmp[:tmp.rfind('/')], 'residue_scores.csv'), 'rb') as csv_import:
...
class create_png_and_cif_of_compound(QtCore.QThread):
    def __init__(self, external_software, initial_model_directory, compound_list, database_directory, data_source_file,
                 todo, ccp4_scratch_directory, xce_logfile, max_queue_jobs, restraints_program):
        QtCore.QThread.__init__(self)
    ...
    def run(self):
        ...
        for item in self.compound_list:
            ...
            if self.todo == 'ALL' or self.todo == 'SELECTED':
                ...
                if os.path.isfile(
                        os.path.join(self.initial_model_directory, sampleID, compoundID.replace(' ', '') + '.pdb')):
                    os.system('/bin/rm ' + os.path.join(self.initial_model_directory, sampleID,
                ...
                if os.path.isfile(
                        os.path.join(self.initial_model_directory, sampleID, compoundID.replace(' ', '') + '.png')):
                    os.system('/bin/rm ' + os.path.join(self.initial_model_directory, sampleID,
            ...
            if not os.path.isfile(
                    os.path.join(self.initial_model_directory, sampleID, 'compound', compoundID + '.smiles')):
            ...
            if not os.path.isfile(
                    os.path.join(self.initial_model_directory, sampleID, compoundID.replace(' ', '') + '.cif')) \
        ...
        self.Logfile.insert(
            'created input scripts for ' + str(counter) + ' ACEDRG jobs in ' + self.ccp4_scratch_directory)
        os.chdir(self.ccp4_scratch_directory)
        ...
        if counter > 1:
            ...
            if os.path.isdir('/dls'):
                if self.external_software['qsub_array']:
                    ...
                    self.Logfile.insert(
                        '         qsub -P labxchem -t 1:{0!s} -tc {1!s} acedrg_master.sh'.format(str(counter),
                                                                                                 self.max_queue_jobs))
                    os.system('qsub -P labxchem -t 1:{0!s} -tc {1!s} acedrg_master.sh'.format(str(counter),
                                                                                              self.max_queue_jobs))
                else:
                    self.Logfile.insert(
...
class run_dimple_on_all_autoprocessing_files(QtCore.QThread):
    def __init__(self, sample_list, initial_model_directory, external_software, ccp4_scratch_directory,
                 database_directory, data_source_file, max_queue_jobs, xce_logfile, remote_submission,
        ...
        for n, item in enumerate(self.sample_list):
            ...
            if 'dimple_rerun_on_selected_file' in visit_run_autoproc:
                additional_cmds = (
                    ...
                    ' {0!s} {1!s} {2!s}\n'.format(os.path.join(self.database_directory, self.data_source_file), xtal,
            ...
            Cmds = (
                ...
                'export XChemExplorer_DIR="' + os.getenv('XChemExplorer_DIR') + '"\n'
                                                                                '\n'
                                                                                'cd %s\n' % os.path.join(
                    self.initial_model_directory, xtal, 'dimple', visit_run_autoproc) +
                '\n'
            ...
            if os.path.isdir('/dls'):
                if
            self.external_software['qsub_array']:
            Cmds = (
            ...
            self.Logfile.insert('using the following command:')
            self.Logfile.insert(
                'qsub -P labxchem -t 1:{0!s} -tc {1!s} dimple_master.sh'.format(str(n + 1), self.max_queue_jobs))
            if self.using_remote_qsub_submission:
                os.system(str(self.remote_qsub_submission).replace("qsub'", str(
                    'cd ' + str(os.getcwd()) + '; ' + 'qsub -P labxchem -t 1:{0!s} -tc {1!s} dimple_master.sh'
                    .format(str(n + 1), self.max_queue_jobs))) + "'")

            else:
                os.system('qsub -P labxchem -t 1:{0!s} -tc {1!s} dimple_master.sh'.format(str(n + 1),
                                                                                          self.max_queue_jobs))
            else:
            self.Logfile.insert(
                "cannot start ARRAY job: make sure that 'module load global/cluster' is in your .bashrc or .cshrc file")
...
class run_dimple_on_all_autoprocessing_files_new(QtCore.QThread):
    def __init__(self, sample_list, initial_model_directory, external_software, ccp4_scratch_directory,
                 database_directory, data_source_file, max_queue_jobs, xce_logfile):
        QtCore.QThread.__init__(self)
        self.sample_list = sample_list
        self.initial_model_directory = initial_model_directory
        self.external_software = external_software
    ...
    def prepare_phenix_ligand_pipeline_shell_script(self, xtal, visit_run_autoproc, mtzin, ref_pdb, ref_mtz, ref_cif):
        ...
        Cmds = (
            ...
            'export XChemExplorer_DIR="' + os.getenv('XChemExplorer_DIR') + '"\n'
                                                                            '\n'
                                                                            'cd %s\n' % os.path.join(
                self.initial_model_directory, xtal, 'phenix.ligand_pipeline') +
            '\n'
            'source $XChemExplorer_DIR/setup-scripts/xce.setup-sh\n'
            '\n'
            'module unload ccp4\n'
    ...
    def prepare_pipedream_shell_script(self, xtal, visit_run_autoproc, mtzin, ref_pdb, ref_mtz, ref_cif):
        ...
        Cmds = (
            ...
            'export XChemExplorer_DIR="' + os.getenv('XChemExplorer_DIR') + '"\n'
                                                                            '\n'
                                                                            'cd %s\n' % os.path.join(
                self.initial_model_directory, xtal, 'pipedream') +
            '\n'
            'source $XChemExplorer_DIR/setup-scripts/xce.setup-sh\n'
            '\n'
            + ccp4_scratch +
            ...
            'EOF\n'
            '\n'
            '$CCP4/libexec/python ' + os.path.join(os.getenv('XChemExplorer_DIR'), 'helpers',
                                                   'update_data_source_for_new_dimple_pdb.py') +
            ' {0!s} {1!s} {2!s}\n'.format(os.path.join(self.database_directory, self.data_source_file), xtal,
                                          self.initial_model_directory) +
            '\n'
    ...
    def prepare_dimple_shell_script(self, xtal, visit_run_autoproc, mtzin, ref_pdb, ref_mtz, ref_cif):
        ...
        if os.path.isfile(ref_mtz):
            ...
            if 'FreeR_flag' not in mtz_column_dict['RFREE']:
                ...
                if mtz_column_dict['RFREE'] != []:
                    self.Logfile.insert(
                        'found Rfree set with other column name though: {0!s}'.format(str(mtz_column_dict['RFREE'])))
                    self.Logfile.insert('try renaming Rfree column to FreeR_flag with CAD!')
    ...
    def run_script(self):
        ...
        if os.path.isdir('/dls'):
            if self.external_software['qsub_array']:
                ...
                self.Logfile.insert('using the following command:')
                self.Logfile.insert(
                    'qsub -P labxchem -t 1:{0!s} -tc {1!s} {2!s}_master.sh'.format(str(self.n), self.max_queue_jobs,
                                                                                   self.pipeline))
                os.system(
                    'qsub -P labxchem -t 1:{0!s} -tc {1!s} {2!s}_master.sh'.format(str(self.n), self.max_queue_jobs,
                                                                                   self.pipeline))
...
class remove_selected_dimple_files(QtCore.QThread):
    ...
    def run(self):
        ...
        for n, xtal in enumerate(self.sample_list):
            ...
            if os.path.isdir(
                    os.path.join(self.initial_model_directory, xtal, 'dimple', 'dimple_rerun_on_selected_file')):
                os.chdir('dimple')
...
class start_COOT(QtCore.QThread):
    ...
    def run(self):
        ...
        os.system('cd {0!s}\ncoot --no-guano --no-state-script --script {1!s}'.format(os.getenv('HOME'), os.path.join(
            os.getenv('XChemExplorer_DIR'), 'lib', self.pylib)))
...
class save_autoprocessing_results_to_disc(QtCore.QThread):
            ...
            for n, entry in enumerate(self.data_collection_dict[sample]):
                if entry[0] == 'logfile':
                    ...
                    if outcome.startswith('Failed'):
                        continue
                    self.emit(QtCore.SIGNAL('update_status_bar(QString)'),
                              'writing all files from data processing to project folder -> ' + sample + ', visit: ' + visit + ', run: ' + run + ', program: ' + autoproc)
                    ...
                    if path_to_dimple_pdbfile != '':
                        ...
                        if not os.path.isdir(os.path.join(self.initial_model_directory, sample, 'dimple',
                            ...
                            os.mkdir(os.path.join(self.initial_model_directory, sample, 'dimple',
                                                  visit + '-' + run + autoproc))
                        dimple_destination = os.path.join(self.initial_model_directory, sample, 'dimple',
                                                          visit + '-' + run + autoproc)

                    if self.processed_data_to_copy == 'mtz_log_only':
                        path_to_logfile, path_to_mtzfile, mtz_filename = self.copy_mtz_and_logfiles_only(sample,
                                                                                                         autoproc, run,
                                                                                                         visit,
                                                                                                         path_to_procdir,
                                                                                                         path_to_logfile,
                                                                                                         ...
                                                                                                         mtz_filename,
                                                                                                         log_filename)
                        db_dict['DataProcessingPathToLogfile'] = os.path.join(self.initial_model_directory, sample,
                                                                              path_to_logfile)
                        db_dict['DataProcessingPathToMTZfile'] = os.path.join(self.initial_model_directory, sample,
                                                                              path_to_mtzfile)
                        self.copy_and_link_selected_dimple_files(dimple_destination, sample, path_to_dimple_mtzfile,
                                                                 path_to_dimple_pdbfile)
                    ...
                    if entry[7] == selected_processing_result:
                        ...
                        tmp_dict = {}
                        tmp_dict['RefinementMTZfree'], tmp_dict['DimpleRcryst'], tmp_dict['DimpleRfree'], tmp_dict[
                            'RefinementOutcome'], tmp_dict[
                            'RefinementSpaceGroup'] = self.link_mtz_log_files_to_sample_directory(sample, autoproc, run,
                                                                                                  visit,
                                                                                                  path_to_procdir,
                                                                                                  path_to_logfile,
                                                                                                  path_to_mtzfile,
    ...
    def copy_and_link_selected_dimple_files(self, dimple_destination, sample, path_to_dimple_mtzfile,
                                            path_to_dimple_pdbfile):
        # dimple files
    ...
    def link_mtz_log_files_to_sample_directory(self, sample, autoproc, run, visit, path_to_procdir, path_to_logfile,
                                               path_to_mtzfile, mtz_filename, log_filename, dimple_destination):
        mtzfree = ''
        Rcryst = ''
        Rfree = ''
        refinement_stage = ''
        spg = ''
        relative_dimple_destination = '.' + dimple_destination.replace(
            os.path.join(self.initial_model_directory, sample), '')
        ...
        if not os.path.isfile(
                os.path.join(self.initial_model_directory, sample, 'dimple', 'dimple_rerun_on_selected_file', 'dimple',
                             'final.pdb')):
    ...
    def copy_mtz_and_logfiles_only(self, sample, autoproc, run, visit, path_to_procdir, path_to_logfile,
                                   path_to_mtzfile, mtz_filename, log_filename):
...
class read_autoprocessing_results_from_disc(QtCore.QThread):
                        ...
                        if isinstance(float(entry[6]['DataProcessingUnitCellVolume']), float):
                            for reference_file in self.reference_file_list:
                                if not reference_file[4] == 0:
                                    unitcell_difference = round((math.fabs(
                                        reference_file[4] - float(entry[6]['DataProcessingUnitCellVolume'])) /
                                                                 reference_file[4]) * 100, 1)
                                    ...
                                    if unitcell_difference < 5 and reference_file[3] == entry[6][
                        ...
                        try:
                            ...
                            try:
                                ranking = (float(entry[6]['DataProcessingUniqueReflectionsOverall']) * \
                                           ...
                                           float(entry[6]['DataProcessingIsigOverall'])) / float(
                                    entry[6]['DataProcessingUnitCellVolume'])
                                select_stage_three_list.append([index, ranking])
                            except ValueError:
                                continue
        if not select_stage_three_list == []:
            ...
            if entry[0] == 'logfile':
                if entry[7] == best_file_index:
                    ...
                    try:
                        ...
                        if os.path.isdir(os.path.join(self.initial_model_directory, xtal, 'autoprocessing',
                                                      visit + '-' + run + autoproc)):
    ...
    def select_best_dataset(self):
        ...
        for xtal in sorted(self.data_collection_dict):
            self.emit(QtCore.SIGNAL('update_status_bar(QString)'),
                      'Step 2 of 2: selecting "best" aimless logfile ->' + xtal)
    ...
    def parse_file_system(self):
        ...
        if self.data_collection_dict == {}:
            if os.path.isfile(self.data_collection_summary_file):
                self.emit(QtCore.SIGNAL('update_status_bar(QString)'),
                          'unpickling: ' + self.data_collection_summary_file)
            ...
            for collected_xtals in sorted(glob.glob(os.path.join(visit_directory, 'processed', self.target, '*'))):
                ...
                for runs in sorted(glob.glob(collected_xtals + '/*')):
                    ...
                    if not image_files_in_list:
                        ...
                        for image in sorted(
                                glob.glob(os.path.join(visit_directory, 'jpegs', self.target, xtal, '*t.png'))):
                            if run in image:
                        ...
                        while image_counter < 4:
                            image_file = open(
                                os.path.join(os.getenv('XChemExplorer_DIR'), 'image', 'IMAGE_NOT_AVAILABLE.png'), "rb")
                            image_string = base64.b64encode(image_file.read())
                        ...
                        if os.path.isfile(os.path.join(visit_directory, 'jpegs', self.target, xtal, run + '.png')):
                            if os.stat(os.path.join(visit_directory, 'jpegs', self.target, xtal,
                                                    run + '.png')).st_size != 0:
                                image_file = open(
                                    os.path.join(visit_directory, 'jpegs', self.target, xtal, run + '.png'), "rb")
                            else:
                                image_file = open(
                        ...
                        if os.path.isfile(
                                os.path.join(visit_directory, 'jpegs', self.target, xtal, run + 'index.html')):
                    ...
                    for file_name in glob.glob(
                            os.path.join(visit_directory, 'processed', protein_name, xtal, run, 'xia2', '*', 'LogFiles',
                                         '*aimless.log')):
                        db_dict = {'DataCollectionVisit': visit,
                                   ...
                                   'DataProcessingDirectoryOriginal': '/'.join(
                                       file_name.split('/')[:len(file_name.split('/')) - 2])}
                        ...
                        for n, entry in enumerate(self.data_collection_dict[xtal]):
                            if entry[0] == 'logfile' and entry[1] == visit and entry[2] == run and entry[4] == autoproc:
                                ...
                                if os.path.isfile(os.path.join(self.initial_model_directory, xtal, 'dimple',
                                                               visit + '-' + run + autoproc, 'dimple', 'final.pdb')):
                                    dimple_file = os.path.join(self.initial_model_directory, xtal, 'dimple',
                                                               visit + '-' + run + autoproc, 'dimple', 'final.pdb')
                                    ...
                                    db_dict_old['DataProcessingPathToDimpleMTZfile'] = dimple_file.replace('.pdb',
                                                                                                           '.mtz')
                        ...
                        if not found_autoproc:  # i.e. this run is not in pkl file yet
                            ...
                            # only then start looking into the regular processed folder
                                    ...
                                    os.path.join(visit_directory, 'processed', protein_name, xtal, run, 'xia2',
                                                 autoproc, 'dimple', 'final.pdb')):
                                dimple_file = os.path.join(visit_directory, 'processed', protein_name, xtal, run,
                                                           'xia2', autoproc, 'dimple', 'final.pdb')
                                pdb_info = parse().PDBheader(dimple_file)
                                db_dict['DataProcessingPathToDimplePDBfile'] = dimple_file
                                db_dict['DataProcessingPathToDimpleMTZfile'] = dimple_file.replace('.pdb', '.mtz')
                                db_dict['DataProcessingRcryst'] = pdb_info['Rcryst']
                            ...
                            self.data_collection_dict[xtal].append(
                                ['logfile', visit, run, timestamp, autoproc, file_name, db_dict, aimless_index, False,
                                 False])
                            aimless_index += 1
                    ...
                    if os.path.isfile(os.path.join(runs, 'fast_dp', 'aimless.log')):
                        ...
                        for n, entry in enumerate(self.data_collection_dict[xtal]):
                            #                        for entry in self.data_collection_dict[xtal]:
                            if len(entry) >= 9:
                                if entry[0] == 'logfile' and entry[1] == visit and entry[2] == run and entry[
                                    4] == autoproc:
                                    found_autoproc = True
                                    # need to check this because user may have run this later and otherwise he would need to delete pkl file to pick it up
                                    if os.path.isfile(os.path.join(self.initial_model_directory, xtal, 'dimple',
                                                                   visit + '-' + run + autoproc, 'dimple',
                                                                   'final.pdb')):
                                        dimple_file = os.path.join(self.initial_model_directory, xtal, 'dimple',
                                                                   visit + '-' + run + autoproc, 'dimple', 'final.pdb')
                                        pdb_info = parse().PDBheader(dimple_file)
                    ...
                    if os.path.isfile(os.path.join(runs, 'autoPROC', 'ap-run', 'aimless.log')):
                        ...
                        if os.path.isfile(os.path.join(runs, 'autoPROC', 'ap-run', 'truncate-unique.mtz')):
                            db_dict['DataProcessingPathToMTZfile'] = os.path.join(runs, 'autoPROC', 'ap-run',
                                                                                  'truncate-unique.mtz')
    ...
    def parse_file_system_NEW(self):
        ...
        for visit_directory in sorted(self.visit_list):
            ...
            self.emit(QtCore.SIGNAL('update_status_bar(QString)'),
                      'parsing ' + os.path.join('/dls_sw', beamline, 'logs') + ' for GDA logiles')
            ...
            for collected_xtals in sorted(glob.glob(os.path.join(current_directory, '*'))):
                ...
                if xtal in gda_pin_dict:
                    ...
                    self.data_source.execute_statement(
                        "update mainTable set DataCollectionPinBarcode ='{0!s}' where CrystalName = '{1!s}'".format(
                            gda_pin_id, xtal))
                    ...
                    if visit_directory != self.initial_model_directory:
                        ...
                        for file_name in glob.glob(
                            ...
                            if not found_autoproc:  # i.e. this run is not in pkl file yet
                                ...
                                #       [9]: the moment the user changes the selection manully this changes to True
                                self.data_collection_dict[xtal].append(
                                    ['logfile', visit, run, timestamp, autoproc, file_name, db_dict, aimless_index,
                                     False, False])
                                aimless_index += 1
                                ...
                                if len(entry) >= 9:
                                    if entry[0] == 'logfile' and entry[1] == visit and entry[2] == run and entry[
                                        ...
                                        if os.path.isfile(os.path.join(self.initial_model_directory, xtal, 'dimple',
                                            ...
                                            db_dict_old = self.data_collection_dict[xtal][n][6]
                                            db_dict_old['DataProcessingPathToDimplePDBfile'] = dimple_file
                                            db_dict_old['DataProcessingPathToDimpleMTZfile'] = dimple_file.replace(
                                                '.pdb', '.mtz')
                                            db_dict_old['DataProcessingRcryst'] = pdb_info['Rcryst']
                                            db_dict_old['DataProcessingRfree'] = pdb_info['Rfree']
                                            self.data_collection_dict[xtal][n][6] = db_dict_old
                        ...
                        # first for xia2 runs
                        for file_name in glob.glob(
                                os.path.join(current_directory, xtal, 'processed', run, '*', 'LogFiles',
                                             '*aimless.log')):
                            db_dict = {'DataCollectionVisit': visit,
                                       'DataCollectionBeamline': beamline,
                                       'DataCollectionDate': timestamp,
...
class read_pinIDs_from_gda_logs(QtCore.QThread):
    ...
    def run(self):
        ...
        self.Logfile.insert(
            "will start parsing of current gda logfile at line {0!s}".format(str(self.gda_log_start_line)))
...
class choose_autoprocessing_outcome(QtCore.QThread):
        ...
        for sample in sorted(self.allSamples):
            if self.db.autoprocessing_result_user_assigned(sample) and not self.rescore:
                self.Logfile.warning(
                    '{0!s}: user has manually selected auto-processing result; will NOT auto-select!'.format(sample))
                continue
            elif self.rescore:
            ...
            try:
                if isinstance(float(resultDict['DataProcessingUnitCellVolume']), float):
                    ...
                    for reference_file in self.reference_file_list:
                        if not reference_file[4] == 0:
                            unitcell_difference = round((math.fabs(
                                reference_file[4] - float(resultDict['DataProcessingUnitCellVolume'])) / reference_file[
                                                             4]) * 100, 1)
                            self.Logfile.insert(resultDict['DataProcessingProgram'] + ': ' +
                                                str(unitcell_difference) + '% difference -> pg(ref): ' +
                                                reference_file[5] + ' -> pg(mtz): ' + resultDict[
                                                    'DataProcessingPointGroup'])
                            if unitcell_difference < self.acceptable_unitcell_volume_difference and reference_file[5] == \
                                    resultDict['DataProcessingPointGroup']:
                                self.Logfile.insert(
    ...
    def selectResultsWithAcceptableLowResoRmerge(self, dbList):
        self.Logfile.insert(
            'checking if MTZ files have acceptable low resolution Rmerge values (currently set to %s)' % str(
...
class read_write_autoprocessing_results_from_to_disc(QtCore.QThread):
    ...
    def getExistingSamples(self):
        ...
        allEntries = self.db.execute_statement(
            'select CrystalName,DataCollectionVisit,DataCollectionRun,DataProcessingProgram from collectionTable where DataCollectionOutcome = "success"')
    ...
    def copyMTZandLOGfiles(self, xtal, run, autoproc, mtzfile, logfile):
        ...
        if os.path.isfile(mtzfile[mtzfile.rfind('/') + 1:]):
            mtzNew = os.path.join(self.projectDir, xtal, 'autoprocessing', self.visit + '-' + run + autoproc,
        ...
        if os.path.isfile(logfile[logfile.rfind('/') + 1:]):
            logNew = os.path.join(self.projectDir, xtal, 'autoprocessing', self.visit + '-' + run + autoproc,
    ...
    def copyJPGs(self, xtal, run):
        for img in glob.glob(os.path.join(self.processedDir.replace('processed', 'jpegs'), xtal, run + '*t.png')):
            if not os.path.isfile(
                    os.path.join(self.projectDir, xtal, 'jpg', self.visit + '-' + run, img[img.rfind('/') + 1:])):
    ...
    def findJPGs(self, xtal, run):
        ...
        for n, img in enumerate(
                glob.glob(os.path.join(self.projectDir, xtal, 'jpg', self.visit + '-' + run, '*t.png'))):
                ...
                for item in self.toParse:
                    ...
                    for folder in glob.glob(procDir):
                        ...
                        if self.alreadyParsed(xtal, current_run, autoproc):
                            continue
                        db_dict = self.readProcessingResults(xtal, folder, logfile, mtzfile, timestamp, current_run,
                                                             autoproc)
