...
class XChemExplorer(QtGui.QApplication):
    ...
    def check_for_new_autoprocessing_or_rescore(self, rescore_only):
        ...
        print 'ijfeugheughjjhjjjjjjjjjjjjjj'
        start_thread = False
        if rescore_only:
...
#            self.update_all_tables()
    ...
    def create_widgets_for_autoprocessing_results_only(self):
...
#            column_name)
        ...
        # 1.) get all crystals that were collected during the visit
#        visit, beamline = XChemMain.getVisitAndBeamline(self.beamline_directory)
#        collectedXtals = self.db.collected_xtals_during_visit(visit)

        # 2.) for every sample, get information about the latest crystal alignment images
        crystalImageDict = self.db.getCrystalImageDict(visit,collectedXtals)

        # 3.) get data collection results for each xtal in visit from mainTable
...
#
    ...
    def get_sample_list_from_table(self,table):
        ...
        for row in xrange(0, allRows):
            sample_id = str(table.item(row, 1).text())
        return sorted(sampleList)

    def update_row_in_table(self,xtal,row,db_dict,table,columns_to_show):
    ...
    def populate_datasets_summary_table(self):
        ...
        for xtal in sorted(collectedXtalsDict):
            if xtal not in samples_in_table:
                ...
                self.update_row_in_table(xtal, row, db_dict, self.datasets_summary_table, self.datasets_summary_table_columns)

        self.datasets_summary_table.resizeRowsToContents()
        self.datasets_summary_table.resizeColumnsToContents()
        self.datasets_summary_table.setSelectionBehavior(QtGui.QAbstractItemView.SelectRows)
        self.datasets_summary_table.cellClicked.connect(self.show_results_from_all_pipelines)
...
#        indexes = self.datasets_summary_table.selectionModel().selectedRows()
#        for index in sorted(indexes):
#            print 'index',index
#            print 'index.row()', index.row()
#            print "sampleID:",self.datasets_summary_table.item(index.row(), 0).text()
#            xtal = self.datasets_summary_table.item(index.row(), 0).text()
#            self.db.all_autoprocessing_results_for_xtal_as_dict(xtal)
...
#            xtal = self.datasets_summary_table.item(index.row(), 0).text()
        ...
        for db_dict in dbList:
            row = data_collection_table.rowCount()
            data_collection_table.insertRow(row)
            self.update_row_in_table(xtal, row, db_dict, data_collection_table, column_name)
        self.data_collection_table_popup(data_collection_table)

    def data_collection_table_popup(self,data_collection_table):
        msgBox = QtGui.QMessageBox()
        msgBoxLayout = msgBox.layout()
        vbox = QtGui.QVBoxLayout()
        vbox.addWidget(data_collection_table)
        msgBoxLayout.addLayout(vbox, 0, 0)
        msgBox.addButton(QtGui.QPushButton('Cancel'), QtGui.QMessageBox.RejectRole)
